defmodule GuardedStruct.AshResource.Change do
  ...
  path. Users with `require_atomic? true` on an action must either set
  `require_atomic? false` or skip this change for that action.
  """
  def atomic(_changeset, _opts, _context) do
    {:not_atomic,
     "GuardedStruct.AshResource.Change runs an imperative sanitize/validate " <>
       "pipeline; not safe to express as atomic SQL."}
  end
